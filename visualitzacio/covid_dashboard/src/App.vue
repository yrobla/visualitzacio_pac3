    ...
    <div class="row">
        ...
        <div id="main_app">
            ...
            <div class="filters-box">
                <p>
                    <button v-on:click="resetFilters">Esborrar filtres</button>
                </p>
                <div class="selector-check">
                    ...
                    <li v-for="region in data.regions" v-bind:key="region">
                ...
                <div class="selector-check">
                    ...
                    <li v-for="age in data.ages" v-bind:key="age">
    ...
    methods: {
        ...
        setLineChartDataMonth() {
            ...
            if (this.selectedAges.length > 0) {
                ...
                for (i = 0; i < this.selectedAges.length; i++) {
            ...
            for (i = 1; i < covid_data.length; i++) {
                ...
                if (data_month == filter_month && data_year == filter_year) {
                    if ((data_month != current_month || data_year != current_year || data_day != current_day) && current_month == filter_month && current_year == filter_year) {
                        categories.push({
                            "label": current_day + "/" + current_month
                        });

                        if (group_criteria == "") {
                            datasets["Total"].push({
                                "value": current_values["Total"]
                            });
                            current_values["Total"] = 0;
                        } else {
        ...
        fillBarChartData: function(current_period, covid_data) {
            ...
            var llista_regions = Object.keys(grouped_regions).map(function(key) {
                return [key, grouped_regions[key]];
            });
            llista_regions.sort(function(first, second) {
                return second[1] - first[1];
            });
            var final_regions = [];
            for (i = 0; i < llista_regions.length; i++) {
                final_regions.push({
                    "label": llista_regions[i][0],
                    "value": llista_regions[i][1]
                });
            }
